...
function moveLeft(){
    ...
    if(cur.some(index=>square[index+curPosition].classList.contains("taken")))
    {
        curPosition+=1;
    }
    shapes_function();
...
{
    remove_shape();
    let right=cur.some(index=>((index+curPosition)%GRID_WIDTH===GRID_WIDTH-1));
...
function display_score(){
    ...
    {
        let rows=[i,i+1,i+2,i+3,i+4,i+5,i+6,i+7,i+8,i+9];
...
function game_over(){
    ...
    if(cur.some(index=>square[curPosition+index].classList.contains('taken')))
    {
